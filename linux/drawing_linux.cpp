		...
		for (size_t i = 0; i < gDrawVector.size(); i++)
		{
			if (ImGui::TreeNode(std::to_string(i).c_str()))
			{
				...
				if (element.mType == DrawListElement::Type::DRAW_LIST_ELEMENT_TYPE_SPRITE)
				{
					...
					ImGui::Text("Texture Position: %d %d %d %d", spriteElement->mTexturePosition.topLeft.x, spriteElement->mTexturePosition.topLeft.y, spriteElement->mTexturePosition.bottomRight.x, spriteElement->mTexturePosition.bottomRight.y);
...
static void imguiPrimsWindowsDrawingData()
{
	if (ImGui::TreeNode("Prism Windows Drawing Data"))
	{
		...
		ImGui::Text("Transformation Matrix: %p", (void*)&gPrismWindowsDrawingData.mTransformationMatrix);
		ImGui::Text("Effect Stack: %p", (void*)&gPrismWindowsDrawingData.mEffectStack);
		...
		ImGui::Text("Palettes: %u %u %u %u", gPrismWindowsDrawingData.mPalettes[0], gPrismWindowsDrawingData.mPalettes[1], gPrismWindowsDrawingData.mPalettes[2], gPrismWindowsDrawingData.mPalettes[3]);
		ImGui::TreePop();
	}
}
