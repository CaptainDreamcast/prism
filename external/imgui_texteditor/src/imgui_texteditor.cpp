...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_Z))
			...
			Redo();
		else if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGuiKey_S))
			Save();
...
void TextEditor::SetPath(const std::string& path)
{
	mPath = path;
}
bool TextEditor::CanSave() const
{
	return !mPath.empty() && !mReadOnly;
}
void TextEditor::Save() const
{
	...
	{
		prism::Buffer b = prism::makeBuffer((void*)textToSave.c_str(), uint32_t(textToSave.size() - 1));
		prism::bufferToFile(mPath.c_str(), b);
	}
}
}